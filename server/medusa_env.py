...
class MedusaEnv(Environment[MedusaAction, MedusaObservation, MedusaState]):
    ...
    def _do_clean_column(
        ...
        for acol, raw_op in today_anomalies:
            mapped_op = op_map.get(raw_op, raw_op)
            if col == acol and op == mapped_op:
    ...
    def _do_evolve_silver_schema(
        ...
        # Mark evolve anomaly as handled
        today_anomalies = self._state.day_anomalies.get(self._state.current_day, [])
        for acol, op in today_anomalies:
            if op == "evolve":
                if (acol, "evolve") not in self._state.cleaned_columns_today:
                    self._state.cleaned_columns_today.append((acol, "evolve"))
    ...
    def _do_quarantine_rows(
        ...
        today_anomalies = self._state.day_anomalies.get(self._state.current_day, [])
        for acol, op in today_anomalies:
            if op == "quarantine" and acol.lower() in condition.lower():
                reward += 0.5
                if (acol, "quarantine") not in self._state.cleaned_columns_today:
                    self._state.cleaned_columns_today.append((acol, "quarantine"))
                break
    ...
    # ---------------------------------------------------------------------------
    # LLM & SFT Utilities (v4.0 Extension)
    # ---------------------------------------------------------------------------

    def generate_llm_prompt(self) -> str:
        """Translates the current environment state into a concise NLP prompt string
        ...
        if not s.unhandled_anomalies_today:
            ...
            prompt.append(f"Trap detected: {s.trap_type}" if s.trap_type else "Normal corruption observed.")
            for col, ops in s.unhandled_anomalies_today.items():
                prompt.append(f"- Column '{col}': requires ({', '.join(ops)})")
                
        # Unhandled schema drift?
        if s.new_schema_cols and not s.did_evolve_schema:
            prompt.append(f"- Schema Drift Detected: {s.new_schema_cols} appear in the source but not the contract. Requires EVOLVE_SILVER_SCHEMA.")
    ...
    @staticmethod
    def parse_llm_action(text: str) -> MedusaAction:
        """Safely parses raw text generated by an LLM into a MedusaAction without crashing."""
        import json
        import re
        
        # Strip markdown syntax if the LLM output ```json ... ```
        ...
        try:
            ...
            # Prevent environment / training loop crash
            return MedusaAction(
                action="DO_NOTHING", 
                params={"error": "LLM produced invalid JSON syntax", "raw_text": text[:50]}
            )
