...
N_OBS = 16   # 16 env features (no manual episode flags)
...
CURRICULUM: List[Tuple[int, List[int]]] = [
    ...
    (400, [0, 1, 3, 4, 6, 8, 9, 10]),       # Phase 2 — easy + medium
    (800, list(range(12))),                 # Phase 3 — all 12
]
...
LAMBDA_GAE = 0.95
CLIP_EPS = 0.2
ENTROPY_COEF = 0.25   # Increased to 0.25 for hard unguided exploration
VALUE_COEF = 0.5
MAX_GRAD_NORM = 0.5
N_STEPS = 1024       # Collect 1024 env timesteps (sparse reward horizon)
...
def compute_action_mask(features: List[float], ep_flags: Dict[str, bool]) -> torch.Tensor:
    ...
    Now an unconstrained RL environment — all actions are valid, allowing the agent
    to organically learn consequences from the reward engine and score audits.
    """
...
def augment_obs(obs_list: List[float], ep_flags: Dict[str, bool]) -> List[float]:
    """Return observations directly without manual history augmentation.
    
    The GRU is fully responsible for tracking pipeline sequence. Extolling 
    the RL architecture limits implicit assistance.
    """
    return obs_list
...
def compute_shaping_reward(
    ...
    """Pure Potential-Based Reward Shaping (Ng et al. 1999).
    
    Instead of arbitrarily rewarding actions, calculate the data quality 'potential'
    Phi(s) of the pipeline before and after the transition.
    R = Phi(s') - Phi(s). 
    """
    # Phi = Key Health + Uniqueness + Freshness
    # (Values are constructed such that 1.0 is healthy context)
    phi_before = (
        (1.0 - obs_before[F_NULL_A]) + 
        (1.0 - obs_before[F_NULL_B]) + 
        obs_before[F_UNIQ_B] + 
        (1.0 - obs_before[F_STALE_A]) + 
        (1.0 - obs_before[F_STALE_B])
    )
    
    phi_after = (
        (1.0 - obs_after[F_NULL_A]) + 
        (1.0 - obs_after[F_NULL_B]) + 
        obs_after[F_UNIQ_B] + 
        (1.0 - obs_after[F_STALE_A]) + 
        (1.0 - obs_after[F_STALE_B])
    )
...
def train(
    ...
    Args:
        ...
        resume_ckpt   : Path to checkpoint to load before starting.
    """
    if smoke_test:
        n_iterations = 5
    ...
    for iteration in range(start_iteration + 1, n_iterations + 1):
        ...
        with torch.no_grad():
            while not buffer.is_full():
                ...
                if done:
                    ...
                    # Defeat the Early Commit Exploit (Local Optima trap)
                    if env.state.silver_row_count == 0:
                        terminal_reward = -5.0
                    else:
                        terminal_reward = final_result.score * 10.0
                        
                    norm_reward = normalize_reward(terminal_reward - 0.02 + shaping)
                else:
                    norm_reward = normalize_reward(-0.02 + shaping)
        ...
        # Entropy annealing: force high exploration early, decay to min
        entropy_coef = max(0.02, ENTROPY_COEF * (1.0 - iteration / n_iterations))
